...
function ScanScreen({ setTab, ar, setFaults }) {
  ...
  async function runPipeline(imageFile) {
    ...
    try {
      ...
      const diagPrompt = "You are an expert automotive diagnostic engineer specializing in GCC-market vehicles. Analyze this vehicle inspection sheet image carefully. Read ALL text, fault codes, checked boxes, handwritten notes, and any marks indicating problems. Return ONLY a valid JSON array - no preamble, no markdown fences, no explanation. Raw JSON only. Each object must follow this schema exactly: {id, severity (high/medium/low), zone (engine_bay/front_left_wheel/front_right_wheel/rear_left_wheel/rear_right_wheel/underbody_front/underbody_rear/cabin_dashboard/battery_electrical/fuel_system), code (DTC or null), nameEn, nameAr, fn, immediate, longterm, steps (array), cost}. Severity: high=safety risk do not drive, medium=repair within 2-4 weeks, low=minor repair within 3-6 months. Vehicle: " + make + " " + model + " " + year + ". If the image is not a vehicle inspection sheet or no faults are visible, return an empty array: []";
      ...
      // Try the secure proxy first, fall back to direct API call
      let response;
      ...
      try {
        ...
        // If proxy returns 404 it means api/diagnose.js hasn't been created yet
      ...
      // Handle Anthropic API error responses forwarded by proxy
      if (data.error) {
      ...
      try {
        ...
        fail("The AI returned an unexpected response format. Please try again.");
        return;
      }
  ...
  // ── Error state (permanent  -  shows what went wrong + what to do) ─────────
  if (step==="error") return (
    <div style={{ padding:24, maxWidth:520, margin:"0 auto" }}>
      {previewUrl && (
        <img src={previewUrl} alt="scan" style={{ width:"100%", maxHeight:160, objectFit:"cover", borderRadius:14, marginBottom:16, opacity:0.6 }}/>
      )}
      <div style={{ background:T.critBg, border:"1.5px solid "+T.redDeep, borderRadius:18, padding:24 }}>
        <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:16 }}>
          <span style={{ fontSize:28 }}>❌</span>
          <div style={{ fontSize:17, fontWeight:800, color:T.redDeep }}>{ar?"فشل التحليل":"Analysis Failed"}</div>
        </div>
        ...
        {/* Helpful next steps */}
        <div style={{ fontSize:12, color:T.redDeep, fontWeight:700, marginBottom:8 }}>{ar?"ماذا تفعل الآن:":"What to do:"}</div>
        <div style={{ display:"flex", flexDirection:"column", gap:6 }}>
          {[
            ar?"تأكد من وجود ملف api/diagnose.js في مستودع GitHub":"Make sure api/diagnose.js exists in your GitHub repo (Phase 6 Step 3)",
            ar?"تأكد من إضافة ANTHROPIC_API_KEY في Vercel Environment Variables":"Check ANTHROPIC_API_KEY is added in Vercel Environment Variables",
            ar?"تأكد من وجود رصيد في حسابك على Anthropic":"Make sure you have billing credit at console.anthropic.com",
            ar?"أعد النشر في Vercel بعد أي تغييرات":"Redeploy on Vercel after making any changes",
          ].map((tip, i) => (
            <div key={i} style={{ display:"flex", gap:8, alignItems:"flex-start", background:"rgba(192,57,43,0.08)", borderRadius:8, padding:"8px 10px" }}>
              <span style={{ color:T.redDeep, fontSize:11, marginTop:1, flexShrink:0 }}>{i+1}.</span>
              <span style={{ fontSize:12, color:T.navy, lineHeight:1.5 }}>{tip}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={{ display:"flex", gap:10, marginTop:16 }}>
        <button onClick={()=>{ setErrorMsg(null); setStep("choose"); }} style={{ flex:1, background:T.navy, color:"#fff", border:"none", borderRadius:12, padding:"13px", fontSize:14, fontWeight:700, cursor:"pointer" }}>
          {ar?"حاول مجددًا":"Try Again"}
        </button>
        <button onClick={()=>{ setErrorMsg(null); setStep("vehicle"); }} style={{ flex:1, background:T.card, color:T.textSec, border:"1px solid "+T.border, borderRadius:12, padding:"13px", fontSize:14, fontWeight:600, cursor:"pointer" }}>
